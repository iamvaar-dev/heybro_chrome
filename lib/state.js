...
export async function loadSettings() {
    const s = await chrome.storage.local.get(["geminiKey", "geminiModel", "experimentalMode", "sessions", "activeSessionId"]);
...
export function updateSessionHistory(event) {
    ...
    if (session) {
        ...
        if (event.type === "block" && event.contentType === "user" && session.title === "New Task") {
