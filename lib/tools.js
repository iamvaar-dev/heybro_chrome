...
export const TOOL_HANDLERS = {
    ...
    tap: async (tabId, args) => {
        ...
        const payload = { action: "click", id: args.id, selector: args.selector, text: args.target || args.text, exact: args.exact, partial: args.partial, index: args.index, xpath: args.xpath, href: args.href, sig: args.signature, clickable: true, visible: true };

        async function verifyAfterTap() {
            // Wait for potential navigation or DOM update first
            ...
            if (v.urlIncludes) {
                ...
                Logger.warn({ type: "tap_verify_fail", reason: "url_mismatch", expected: u, actual: await sendToAgent(tabId, { t: "getPageState" }).then(r => r?.state?.url) });
                return false;
            }
            if (v.element) {
                ...
                Logger.warn({ type: "tap_verify_fail", reason: "element_not_found", element: v.element });
                return false;
            }
            return true;
        }
    ...
    type: async (tabId, args) => {
        ...
        const payload = { action: "type", id: args.id, selector: args.selector, text: args.target || args.field || args.label, value: args.value || args.text, append: args.append, mode: args.mode || "set", clearFirst: args.clear !== false, simulate: args.simulate !== false, xpath: args.xpath, sig: args.signature };
