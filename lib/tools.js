...
export function isIgnoredUrl(url) {
    if (!url) return false;
    return /googletagmanager|doubleclick|facebook\.com\/tr|google-analytics|ads\.google/.test(url);
}
...
async function sendToAgent(tabId, msg, options = {}) {
    ...
    const targetFrameId = options.frameId !== undefined ? options.frameId : 0;

    try {
        const pong = await chrome.tabs.sendMessage(tabId, { t: "ping" }, { frameId: targetFrameId });
    ...
    for (let i = 0; i < 3; i++) {
        try {
            return await chrome.tabs.sendMessage(tabId, msg, { frameId: targetFrameId });
        } catch (err) {
...
async function execute(tabId, payload) {
    ...
    // Sort frames: main frame (0) first, then others. Filter out ignored URLs, BUT keep frame 0 always.
    const sortedFrames = frames
        .filter(f => f.frameId === 0 || !isIgnoredUrl(f.url))
        .sort((a, b) => {
    ...
    // If all specific frame attempts failed or were filtered, try the default broadcast
    // (though sendToAgent usually targets top frame, which we already tried if frameId 0 was in list)
    // But if getAllFrames missed something, this is a fallback.
    return await sendToAgent(tabId, { t: "execute", payload });
}
...
export const TOOL_HANDLERS = {
    ...
    type: async (tabId, args) => {
        ...
        let payload = { action: "type", id: args.id, selector: args.selector, text: args.target || args.field || args.label, value: args.value || args.text, append: args.append, mode: args.mode || "set", clearFirst: args.clear !== false, simulate: args.simulate !== false, xpath: args.xpath, sig: args.signature, submit: args.submit || false };
            ...
            try {
                ...
                if (payload.target === "_blank") {
                    ...
                    if (payload.waitForLoad !== false) {
                        // Wait for URL to update (at least hostname match) to avoid returning early on old page
                        ...
                        try {
                            ...
                            while (Date.now() - start < timeout) {
                                try {
                                    const r = await sendToAgent(tabId, { t: "getPageState" });
                                    const current = r?.state?.url;
                                    if (current) {
                                        const curObj = new URL(current);
                                        if (curObj.hostname.includes(targetObj.hostname.replace("www.", ""))) {
                                            break;
                                        }
                                    }
                                } catch { }
                        ...
                        const ready = await waitReady(tabId, timeout);
                        const verified = await verifyAfterNavigate();
