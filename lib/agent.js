...
import { getEls, setRunning, appendBlock, streamText } from './ui.js';
...
export async function startAutoRun() {
    ...
    while (!state.autoStop && step < 25) {
        ...
        if (nc.tool === "done") {
            ...
            // Mark all as done
            currentPlan.forEach(t => t.status = "completed");
        ...
        if (r && r.ok) {
            ...
            if (pendingIdx >= 0) {
                if (nc.tool !== "wait" && nc.tool !== "scroll") {
                    ...
                    if (pendingIdx > 0) currentPlan[pendingIdx - 1].status = "completed";
                }
                // Only show plan update if it changed significantly or rarely
                // User complained about repetitive plans. Let's show it less often.
                // Maybe only if we completed a step?
                if (pendingIdx > 0 && currentPlan[pendingIdx - 1].status === "completed") {
                    appendBlock("Updating progress...", "action");
                    appendBlock(currentPlan, "plan");
                }
            }
        } else {
